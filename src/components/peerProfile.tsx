...
PeerProfile.Phone = () => {
  ...
  const phoneDetails = createMemo(() => {
    ...
    return {
      ...
      formatted: phone ? formatUserPhone(phone) : ''
    };
  });
...
PeerProfile.Username = () => {
  ...
  const backendSelfUsername = createMemo(() => {
    if(!Modes.backend || !context.peerId.isUser() || context.peerId !== rootScope.myId) {
      return;
    }
    const u = backendBootstrapStore.currentUser;
    const n = u && typeof u.username === 'string' ? u.username.trim() : '';
    return n || undefined;
  });

  const mainUsername = createMemo(() => backendSelfUsername() || usernames()?.[0]);
  ...
  return (
    <Show when={!!mainUsername()}>
      ...
      >
        ...
        <Row.Subtitle>{
          backendSelfUsername() ? i18n('Username') :
